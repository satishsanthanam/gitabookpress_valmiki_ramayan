# Version 3.4 - Valmiki Ramayana Markdown to JSON Converter (Balakanda Summary Edition)
# Features:
# - Parses 'balakanda_mistral_ocr.md'
# - Captures Canto summaries as shloka: 0
# - Points audio_file links to 'ramayana/balakanda/{sarga}_{shloka}.mp3'
# - Cleans headers, footnotes, and page numbers

import json
import re

INPUT_MD_FILE = "balakanda_mistral_ocr.md"
OUTPUT_JSON_FILE = "Valmiki_Ramayan_Balakanda.json"
KANDA_NAME = "Bala Kanda"
KANDA_FOLDER = "balakanda"
...
def clean_and_normalize_dataset(raw_dataset):
    ...
    for item in raw_dataset:
        ...
        exp_cleaned = re.sub(r'\*\s*VĀLMĪKI-RĀMĀYAṆA\s*\*|\*\s*BĀLAKĀṆḌA\s*\*|\*\s*Bālakāṇḍa\s\*', '', raw_exp)
        exp_cleaned = re.sub(r'^\s*\d{1,4}\s*$', '', exp_cleaned, flags=re.MULTILINE)
        exp_cleaned = re.sub(r'^\s*\*.*$', '', exp_cleaned, flags=re.MULTILINE)
...
if __name__ == "__main__":
    print("--- [Version 3.4] Balakanda Markdown to JSON Converter ---")
    ...
    print(f"✓ Saved updated dataset with summaries to '{OUTPUT_JSON_FILE}'!")